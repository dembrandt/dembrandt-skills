...
- **Consolidation is a run that produces a table, not a prompt bullet.** Five products give five drifted palettes. Deciding shared baseline vs deliberate per-product override vs accidental drift is the expensive step. Produce that table before any authoring starts. A model asked to decide and author in one pass resolves drift silently and mentions it nowhere.
- **Measured tokens cannot settle direction.** Extraction says what ships. Only a brand, positioning or mission document licenses moving away from it, so every deviation from a measured value cites a line in one of those. No citation, keep the measurement.
- **Verify the authored system, not the transcript.** An authoring run reports success in prose. Export the token set you authored (Figma variables, Penpot tokens, the theme file) and diff it against the extraction it came from. A value that exists only in the design file was invented. A token bound to no shape is dead. Neither shows in the agent's own summary.
- **Figma and Penpot are not the same instruction.** Figma authors variables in collections with modes; Penpot authors token sets with themes. "Three variants" is one collection with three modes in Figma and three sets with a theme each in Penpot. Figma's official MCP reads a file into context; writing variables needs the plugin API. Until a write path is verified end to end, Figma is the tool you read from and reconcile against, and Penpot or code is where the system is authored.
